...
pub fn record_event(
    ...
    let path = logs_dir.join(diagnostic_log_file_name());
    let line = json!({
        "timestamp": timestamp,
        "level": level,
        "event": event,
...
fn diagnostic_log_file_name() -> String {
    format!("aicleaner-diagnostics-{}.jsonl", utc_date_stamp())
}
...
mod tests {
    ...
    #[test]
    fn writes_records_to_daily_jsonl_file() {
        let dir = std::env::temp_dir().join(format!("aicleaner-diagnostics-{}", Uuid::new_v4()));
        ...
        assert_eq!(name.len(), "aicleaner-diagnostics-2026-05-14.jsonl".len());
        let raw = fs::read_to_string(entries[0].path()).expect("read daily log");
        assert_eq!(raw.lines().count(), 3);
        assert!(raw.contains(&task_a));
        assert!(raw.contains(&task_b));

...
export function normalizeRemoteModels(models) {
    const seen = new Set();
    const normalized = [];
...
export function fallbackModelsByEndpoint(endpoint) {
...
export function defaultModelByEndpoint(endpoint) {
    return fallbackModelsByEndpoint(endpoint)[0]?.value || DEFAULT_PROVIDER_MODEL;
}

export function getProviderLabel(endpoint) {
    return PROVIDER_OPTIONS.find((item) => item.value === endpoint)?.label || String(endpoint || '').trim() || 'N/A';
}

export function ensureProviderOptionExists(select, endpoint) {
    if (!select || !endpoint) return;
    const normalized = String(endpoint).trim();
    const exists = Array.from(select.options).some((opt) => opt.value === normalized);
    if (!exists) {
        select.add(new Option(getProviderLabel(normalized), normalized));
    }
}

export function populateProviderOptions(select) {
    if (!select) return;
    for (const option of PROVIDER_OPTIONS) {
        const exists = Array.from(select.options).some((item) => item.value === option.value);
        if (!exists) {
